...
import { safeRoutesAPI, UserFeedbackRequest } from '../services/api';
...
// 🎨 TIPOS DE FEEDBACK CON COLORES Y DESCRIPCIONES
...
export default function CreateFeedbackScreen() {
  ...
  const [newFeedback, setNewFeedback] = useState<UserFeedbackRequest>({
    lat: 0,
    lng: 0,
    tipo: 'SEGURIDAD',
    comentario: '',
  ...
  useEffect(() => {
    console.log('🗺️ CreateFeedback mounting - getting GPS location...');
  ...
  const loadCurrentUser = async () => {
    try {
        ...
        if (storedUser?.username) {
            ...
            console.log('👤 User loaded:', storedUser.username);
        }
    } catch (error) {
  ...
  const nextStep = () => {
    if (step === 'location') {
      ...
      if (!newFeedback.comentario.trim()) {
        Alert.alert('📝 Comentario requerido', 'Por favor describe tu experiencia');
        return;
      }
      setStep('preview');
    }
  ...
  const createFeedback = async () => {
    try {
      ...
      if (newFeedback.lat < 4.0 || newFeedback.lat > 5.0 || 
          newFeedback.lng < -75.0 || newFeedback.lng > -73.0) {
        Alert.alert(
          '🚨 Ubicación Inválida',
      ...
      console.log('🗣️ Creating feedback:', newFeedback);
      
      // 📅 Actualizar fecha antes de enviar
      ...
      // Llamar a la API
      await safeRoutesAPI.createFeedback(feedbackToSend);
      
      Alert.alert(
        '🎉 ¡Realimentación enviada!',
        `Tu feedback sobre "${FEEDBACK_TYPES[newFeedback.tipo as keyof typeof FEEDBACK_TYPES].name}" ha sido registrado exitosamente`,
        [
          {
            text: 'Ver mis feedbacks',
      ...
      Alert.alert('❌ Error', 'No se pudo enviar la realimentación. Intenta nuevamente.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        {Object.entries(FEEDBACK_TYPES).map(([key, type]) => (
          <TouchableOpacity
            ...
            style={[
              ...
              newFeedback.tipo === key && styles.typeCardSelected
            ]}
            onPress={() => setNewFeedback(prev => ({ ...prev, tipo: key }))}
          >
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>➕ Nueva Realimentación</Text>
      ...
      {step === 'location' && (
        ...
        <View style={styles.stepContainer}>
          ...
          <Text style={styles.stepDescription}>
            Toca en el mapa la ubicación exacta de tu experiencia
          </Text>
            ...
            >
              {selectedLocation && (
                <Marker
                  ...
                  title="Mi Realimentación"
                  description={`${FEEDBACK_TYPES[newFeedback.tipo as keyof typeof FEEDBACK_TYPES].name}`}
                />
              )}
            </MapView>
      ...
      {step === 'details' && (
        // 📝 PASO 2: DETALLES DEL FEEDBACK
        <ScrollView style={styles.stepContainer} showsVerticalScrollIndicator={false}>
          <Text style={styles.stepTitle}>📝 Cuéntanos tu experiencia</Text>
          <Text style={styles.stepDescription}>
            Describe lo que experimentaste en este lugar
          </Text>
          ...
          <View style={styles.inputContainer}>
            <Text style={styles.inputLabel}>💬 Tu Experiencia *</Text>
            <TextInput
              style={[styles.textInput, styles.textArea]}
              placeholder="Describe lo que pasó, cómo te sentiste, qué observaste..."
              value={newFeedback.comentario}
              onChangeText={(text) => setNewFeedback(prev => ({ ...prev, comentario: text }))}
              multiline
              numberOfLines={5}
              maxLength={300}
              textAlignVertical="top"
            />
            <Text style={styles.inputHelper}>{newFeedback.comentario.length}/300 caracteres</Text>
          </View>

          {/* ℹ️ Info adicional */}
          <View style={styles.infoContainer}>
            <Text style={styles.infoTitle}>ℹ️ Información adicional</Text>
      ...
      {step === 'preview' && (
        ...
        <ScrollView style={styles.stepContainer} showsVerticalScrollIndicator={false}>
          <Text style={styles.stepTitle}>✅ Confirmar Realimentación</Text>
          <Text style={styles.stepDescription}>
          ...
          <View style={styles.previewCard}>
            <LinearGradient
              colors={FEEDBACK_TYPES[newFeedback.tipo as keyof typeof FEEDBACK_TYPES].gradient}
              style={styles.previewGradient}
            >
              <View style={styles.previewHeader}>
                <Text style={styles.previewIcon}>{FEEDBACK_TYPES[newFeedback.tipo as keyof typeof FEEDBACK_TYPES].icon}</Text>
                <View style={styles.previewInfo}>
                  <Text style={styles.previewType}>{FEEDBACK_TYPES[newFeedback.tipo as keyof typeof FEEDBACK_TYPES].name}</Text>
              ...
              <View style={styles.previewDetails}>
                <View style={styles.previewDetail}>
                  ...
                  <Text style={styles.previewDetailText}>
                    {newFeedback.lat.toFixed(4)}, {newFeedback.lng.toFixed(4)}
                  </Text>
                </View>
                
                <View style={styles.previewDetail}>
                  <Text style={styles.previewDetailIcon}>👤</Text>
                  <Text style={styles.previewDetailText}>{currentUser}</Text>
                </View>
              </View>
            </LinearGradient>
          </View>
          ...
          <View style={styles.miniMapContainer}>
            ...
            <View style={styles.miniMap}>
              <MapView
                ...
                customMapStyle={mapStylePastel}
                region={{
                  latitude: newFeedback.lat,
                  longitude: newFeedback.lng,
                  latitudeDelta: 0.01,
                  longitudeDelta: 0.01,
                }}
                scrollEnabled={false}
      ...
      <View style={styles.buttonContainer}>
        ...
        <TouchableOpacity 
          ...
          onPress={step === 'preview' ? createFeedback : nextStep}
          disabled={isLoading}
        >
          <LinearGradient
          ...
          >
            <Text style={styles.primaryButtonText}>
              {isLoading ? '⏳ Enviando...' : 
               step === 'preview' ? '🎉 Enviar Realimentación' : 'Siguiente →'}
            </Text>
          </LinearGradient>
        </TouchableOpacity>
      </View>
